...
import dev.aurelium.auraskills.sponge.antiafk.handler.PositionHandler;
...
public class EntityA extends Check {
    ...
    public EntityA(CheckType type, AntiAfkManager manager) {
        super(type, manager);
    ...
    public void onXpGain(XpGainEvent event) {
        if (isDisabled() || !(event.getSource() instanceof EntityXpSource)) return;
