...
public class FishingA extends Check {

    private final FacingHandler handler;

    public FishingA(CheckType type, AntiAfkManager manager) {
        super(type, manager);
    ...
    public void onXpGain(XpGainEvent event) {
        if (isDisabled() || !(event.getSource() instanceof FishingXpSource)) return;
